...
def copy_batch_warp(
    ...
    out_features_wp = wp.zeros(
        (batch_size, num_points.max(), in_features.shape[1]),
        device=device,
    )
    ...
    if num_copy_per_thread is None:
        ...
        wp.launch(
            ...
            inputs=[out_features_wp, in_features_wp, row_splits_wp, num_copy_per_thread],
        )
...
def copy_batch_torch(
    ...
    out_features = torch.zeros(
        (row_splits.shape[0] - 1, num_points.max(), in_features.shape[1]),
        dtype=torch.float32,
        device=device,
    )
...
def cat_to_batch(
    ...
    if backend == "torch":
        return copy_batch_torch(in_features, row_splits)
    elif backend == "warp":
        return copy_batch_warp(in_features, row_splits, num_copy_per_thread)
    else:
