...
template <torch::ScalarType ScalarA,
          ...
          torch::ScalarType ScalarOutput,
          torch::ScalarType ScalarAccumulator>
int dispatch_cutlass_gemm_ab_gather(const torch::Tensor &tensor_a,
                                    const torch::Tensor &tensor_b,
                                    const torch::Tensor &tensor_c,
                                    torch::Tensor &tensor_d,
                                    const torch::Tensor &indices_a,
  ...
  return run_cutlass_gemm_ab_gather<ElementA, ElementB, ElementOutput, ElementAccumulator>(
      tensor_a.data_ptr(),
      tensor_b.data_ptr(),
      tensor_c.data_ptr(),
      tensor_d.data_ptr(),
...
torch::Tensor cutlass_gemm_ad_gather_scatter(torch::Tensor tensor_a,
...
torch::Tensor cutlass_gemm_ab_gather(torch::Tensor tensor_a,
                                     torch::Tensor tensor_b,
                                     torch::Tensor tensor_c,
                                     torch::Tensor tensor_d,
                                     torch::Tensor indices_a,
  ...
  TORCH_CHECK(indices_b.size(0) == indices_size,
              "indices_a and indices_b must have same number of rows");
  ...
  if (scalar_a == torch::kFloat16 && scalar_b == torch::kFloat16 && scalar_c == torch::kFloat16 &&
    ...
    // Use float accumulator (ScalarC) while keeping A,B,D in FP16
    if (accumulator_type == torch::kFloat16) {
      status = dispatch_cutlass_gemm_ab_gather<torch::kFloat16,
                                               torch::kFloat16,
                                               torch::kFloat16,
                                               torch::kFloat16>(tensor_a,
                                                                tensor_b,
    ...
    // Pure BF16 path (ibf16obf16) with FP32 accumulator
    if (accumulator_type == torch::kFloat32) {
      status = dispatch_cutlass_gemm_ab_gather<torch::kBFloat16,
                                               torch::kBFloat16,
                                               torch::kBFloat16,
                                               torch::kFloat32>(tensor_a,
                                                                tensor_b,
...
torch::Tensor cutlass_gemm_trAB_gather(torch::Tensor tensor_a,
                                       torch::Tensor tensor_b,
                                       torch::Tensor tensor_c,
                                       torch::Tensor tensor_d,
                                       torch::Tensor indices_a,
  ...
  int M_A = tensor_a.size(0);  // Original A matrix rows
  int K = tensor_a.size(1);    // A columns
  int M_B = tensor_b.size(0);  // Original B matrix rows
  int N = tensor_b.size(1);    // B columns
  int gather_a_size = indices_a.size(0);

  // Check dimension compatibility for A[indices_a, :].T @ B[indices_b, :]
  // A: M_A × K (tall skinny), B: M_B × N (tall skinny)
  // A[indices_a, :]: gather_a_size × K
  // A[indices_a, :].T: K × gather_a_size
  // B[indices_b, :]: gather_a_size × N
  // Result: K × N
...
PYBIND11_MODULE(_C, m) {
  ...
  m.def("cutlass_gemm_ad_gather_scatter",
  ...
  // AB Gather
  m.def("cutlass_gemm_ab_gather",
        &cutlass_gemm_ab_gather,
        "Run CUTLASS GEMM with gather/scatter (half precision inputs and "
        "accumulator)",
        py::arg("tensor_a"),
        py::arg("tensor_b"),
  ...
  // trAB Gather
  m.def("cutlass_gemm_trAB_gather",
