...
//     return n;
// }
// Note: The _part1by2 for 32-bit integers is not directly used by the 64-bit Morton code calculations.
// If it were needed, it would be included. The original Warp code only uses _part1by2_long in _morton_code.
...
__device__ int64_t morton_code_16bit_device(const int* bcoord_point) {
    ...
    int64_t morton = (part1by2_long(uz) << 2) |
                     (part1by2_long(uy) << 1) |
                      part1by2_long(ux);
...
__device__ int64_t morton_code_20bit_device(int coord_x, int coord_y, int coord_z) {
    ...
    return (part1by2_long(uz) << 2) |
           (part1by2_long(uy) << 1) |
            part1by2_long(ux);
}

extern "C" __global__ void assign_order_discrete_16bit_kernel(
    const int* __restrict__ bcoords_data, // Flattened 2D array (num_points, 4), [batch_idx, x, y, z]
    int num_points,
...
extern "C" __global__ void assign_order_discrete_20bit_kernel(
    ...
    int64_t* __restrict__ result_order)
{
    ...
    if (tid < num_points) {
        ...
        result_order[tid] = morton_code_20bit_device(coord_x, coord_y, coord_z);
    }
}
